...
export class ColorValidator {
    hexRegex = /^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$/;
    rgbRegex = /^rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)$/;
    oklchRegex = /^oklch\(\s*([\d.]+%?)\s+([\d.]+)\s+([\d.]+)(?:deg)?\s*\)$/;
    referenceRegex = /^\{([^}]+)\}$/;
    tokenCache = new Map();
    ...
    detectColorFormat(value) {
        ...
        if (trimmedValue.startsWith('rgb(')) {
            ...
            if (rgbMatch) {
                ...
                const errors = [];
                if (red < 0 || red > 255)
                    errors.push(`Red value ${red} out of range (0-255)`);
                if (green < 0 || green > 255)
                    errors.push(`Green value ${green} out of range (0-255)`);
                if (blue < 0 || blue > 255)
                    errors.push(`Blue value ${blue} out of range (0-255)`);
                return {
        ...
        if (trimmedValue.startsWith('oklch(')) {
            ...
            if (oklchMatch) {
                ...
                if (lightness < 0 || lightness > 1)
                    errors.push(`Lightness ${lightness} out of range (0-1)`);
                if (chroma < 0 || chroma > 0.4)
                    errors.push(`Chroma ${chroma} out of range (0-0.4)`);
                if (hue < 0 || hue >= 360)
                    errors.push(`Hue ${hue} out of range (0-359)`);
                return {
                    isValid: errors.length === 0,
                    format: 'oklch',
     ...
     */
    validateColorFormat(value, expectedFormat) {
        const errors = [];
     ...
     */
    validateOKLCHColor(value, errors) {
     ...
     */
    validateOKLCHRange(color, errors) {
        const { l, c, h } = color;
     ...
     */
    validateTokenReference(referencePath, tokenSet, visited = new Set()) {
        const result = {
            path: referencePath,
            resolved: false,
            circularRef: false,
     ...
     */
    validateFormatConsistency(tokens, expectedFormat) {
        const errors = [];
     ...
     */
    validateColorToken(tokenPath, token, tokenSet) {
        const result = {
            token: token.$value,
            tokenPath,
            valid: true,
        ...
        const formatErrors = this.validateColorFormat(token.$value, result.format);
        result.errors.push(...formatErrors);
        // Reference validation
        if (result.format === 'reference' && formatValidation.normalizedValue) {
            const refResult = this.validateTokenReference(formatValidation.normalizedValue, tokenSet);
        ...
        if (result.format !== 'reference' && result.valid) {
            try {
                const oklchColor = oklch(token.$value);
                if (oklchColor) {
