...
export class Phase1PatternAnalyzer {
     ...
     */
    static extractStepNumber(colorKey) {
    ...
    static async analyzePatterns() {
        ...
        const mostConsistent = goodFamilies.reduce((best, current) => current.consistency.smoothness < best.consistency.smoothness ? current : best);
     ...
     */
    static generateHTMLVisualization(analysis) {
        ...
        `;
            ...
            const stepDiffsHTML = family.stepDifferences.map((diff, i) => `<span class="step-diff ${Math.abs(diff - analysis.idealSteppingPattern.averageStepSize) > 0.01 ? 'irregular' : 'good'}">${diff.toFixed(4)}</span>`).join('');
            return `
...
//# sourceMappingURL=phase-1-pattern-analyzer.js.map