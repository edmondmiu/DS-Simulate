...
export class MultiBrandValidator {
     ...
     */
    static extractBrandColors(coreTokens) {
     ...
     */
    static calculateBrandCharacteristics(colors) {
     ...
     */
    static analyzeAccessibility(colors) {
        const validColors = Object.values(colors).filter(color => color && color !== '');
        let aaCompliant = 0;
        let aaaCompliant = 0;
     ...
     */
    static analyzeCrossBrandRelationships(base, logifuture, bet9ja) {
        ...
        const normalizeHueDiff = (diff) => Math.min(diff, 360 - diff);
...
//# sourceMappingURL=multi-brand-validator.js.map