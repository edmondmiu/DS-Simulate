...
export class VisualRegressionTester {
     ...
     */
    static extractAllColors(tokens) {
    ...
    static testVisualRegression() {
        ...
        Object.keys(currentColors).forEach(familyName => {
            ...
            Object.keys(familyColors).forEach(colorKey => {
                ...
                if (actualDeltaE > familyMaxDeltaE)
                    familyMaxDeltaE = actualDeltaE;
                if (actualDeltaE > maxDeltaE)
                    maxDeltaE = actualDeltaE;
                if (actualDeltaE < 2.0) {
     ...
     */
    static isNeutralFamily(familyName) {
     ...
     */
    static simulateNeutralOptimization(hex, colorKey) {
     ...
     */
    static simulateBrandOptimization(hex, colorKey, familyName) {
        // Extract step from color key
        const stepMatch = colorKey.match(/(\d+)$/);
        if (!stepMatch) {
            return { optimized: hex, deltaE: 0 };
     ...
     */
    static getReferenceOptimized(hex, isNeutral) {
        const oklchColor = require('culori').oklch(hex);
        if (!oklchColor)
            return hex;
     ...
     */
    static generateVisualRegressionReport() {
...
//# sourceMappingURL=comprehensive-visual-regression.js.map