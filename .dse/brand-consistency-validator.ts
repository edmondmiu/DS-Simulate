...
import { oklch, deltaE, converter } from 'culori';
...
export class BrandConsistencyValidator {
  private readonly defaultRules: ConsistencyRules = {
  ...
  private harmonyCache = new Map<string, number>();
   ...
   */
  public calculateHueHarmony(hues: number[]): number {
    if (hues.length < 2) return 1.0;
    ...
    for (let i = 0; i < hues.length; i++) {
      for (let j = i + 1; j < hues.length; j++) {
        ...
        if (Math.abs(angleDiff - 0) <= 5) pairScore = 1.0;           // Same hue
        else if (Math.abs(angleDiff - 180) <= this.rules.harmonyTolerances.complementary) pairScore = 0.9;  // Complementary
        else if (Math.abs(angleDiff - 120) <= this.rules.harmonyTolerances.triadic) pairScore = 0.85;       // Triadic
        else if (Math.abs(angleDiff - 240) <= this.rules.harmonyTolerances.triadic) pairScore = 0.85;       // Triadic
        else if (angleDiff <= this.rules.harmonyTolerances.analogous) pairScore = 0.8;                      // Analogous
        else if (Math.abs(angleDiff - 90) <= 15) pairScore = 0.7;    // Square/tetradic
        else if (Math.abs(angleDiff - 270) <= 15) pairScore = 0.7;   // Square/tetradic
        else if (Math.abs(angleDiff - 60) <= 10) pairScore = 0.75;   // Split complementary
        else if (Math.abs(angleDiff - 150) <= 15) pairScore = 0.6;   // Split complementary variation
        else pairScore = Math.max(0, 0.5 - (angleDiff - 30) / 180);  // Decreasing score for discord
   ...
   */
  public validateLightnessProgression(colors: { token: string; oklch: OKLCH }[]): {
    valid: boolean;
    score: number;
    errors: string[];
    suggestions: string[];
   ...
   */
  public validateSemanticColors(brandThemes: BrandTheme[]): BrandConsistencyError[] {
    const errors: BrandConsistencyError[] = [];
    const semanticColors = new Map<string, OKLCH[]>();

    // Collect semantic colors from all brands
    brandThemes.forEach(theme => {
      this.rules.requiredSemanticColors.forEach(semanticRole => {
        const colorKey = Object.keys(theme.colors).find(key => 
          key.toLowerCase().includes(semanticRole.toLowerCase())
        );
   ...
   */
  private calculateHueVariance(hues: number[]): number {
   ...
   */
  public validateBrandHarmony(theme: BrandTheme): {
    harmonyScore: number;
    errors: BrandConsistencyError[];
    suggestions: BrandConsistencySuggestion[];
  } {
    const errors: BrandConsistencyError[] = [];
    const suggestions: BrandConsistencySuggestion[] = [];
    ...
    const colors = Object.entries(theme.colors)
      ...
      .filter((item): item is { token: string; oklch: OKLCH } => item.oklch !== undefined);
   ...
   */
  private generateHarmonicHues(baseHue: number): number[] {
    return [
      baseHue,                           // Base
   ...
   */
  public validateBrandConsistency(brandThemes: BrandTheme[]): BrandConsistencyResult[] {
    const results: BrandConsistencyResult[] = [];

    // Individual brand validation
    brandThemes.forEach(theme => {
      const result: BrandConsistencyResult = {
   ...
   */
  private extractColorRamps(theme: BrandTheme): { name: string; colors: { token: string; oklch: OKLCH }[] }[] {
    const ramps: { name: string; colors: { token: string; oklch: OKLCH }[] }[] = [];
    const rampGroups = new Map<string, { token: string; oklch: OKLCH }[]>();
   ...
   */
  private determineColorRole(token: string): 'primary' | 'secondary' | 'accent' | 'neutral' | 'semantic' | 'ramp' {
    const lowerToken = token.toLowerCase();
    
    if (lowerToken.includes('primary')) return 'primary';
    if (lowerToken.includes('secondary')) return 'secondary';
    if (lowerToken.includes('accent')) return 'accent';
    if (lowerToken.includes('neutral') || lowerToken.includes('gray') || lowerToken.includes('grey')) return 'neutral';
    if (['success', 'error', 'warning', 'info', 'danger'].some(s => lowerToken.includes(s))) return 'semantic';
    if (/\d+$/.test(token)) return 'ramp';
   ...
   */
  public updateRules(newRules: Partial<ConsistencyRules>): void {
   ...
   */
  public getRules(): ConsistencyRules {
    return { ...this.rules };
  }
}
