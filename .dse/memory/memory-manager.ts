...
export class DSEMemoryManager {
  ...
  constructor(basePath: string = '.dse/memory') {
  ...
  loadEpicMemory(epicId: string): EpicMemory | null {
    ...
    return JSON.parse(content) as EpicMemory;
  }
  ...
  loadContextMemory(): ContextMemory | null {
    ...
    const content = readFileSync(filePath, 'utf-8');
    return JSON.parse(content) as ContextMemory;
  }
  ...
  loadRelevantMemory(query: MemoryQuery): MemoryLoadResult {
    ...
    const relevantTasks: TaskMemory[] = [];
  ...
  private filterEpics(epics: EpicMemory[], query: MemoryQuery): EpicMemory[] {
    return epics.filter(epic => {
      ...
      if (query.filters.timeRange) {
        ...
        if (epicDate < startDate || epicDate > endDate) return false;
      }
      ...
      if (query.filters.keywords) {
        ...
        const hasKeywords = query.filters.keywords.some(keyword => 
          epicText.includes(keyword.toLowerCase())
        );
  ...
  private filterPatterns(patterns: Pattern[], query: MemoryQuery): Pattern[] {
    return patterns.filter(pattern => {
      ...
      if (query.filters.keywords) {
        ...
        const hasKeywords = query.filters.keywords.some(keyword => 
          patternText.includes(keyword.toLowerCase())
        );
        if (!hasKeywords) return false;
      }
      ...
      if (pattern.effectiveness < relevanceThreshold) return false;
      
      return true;
    });
  }

  private generateSuggestions(epics: EpicMemory[], tasks: TaskMemory[], patterns: Pattern[]): string[] {
  ...
  private getDefaultContext(): ContextMemory {
    return {
