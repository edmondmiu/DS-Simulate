...
export interface ColorLibraryConfig {
  colorLibrary: {
    colorSpace: 'oklch';
    lightnessRange: {
      min: number; // 0-100, typically 15-95 for accessibility
      max: number;
    };
    chromaRange: {
      primary: number; // 0-0.4, chroma intensity for primary colors
      neutral: number; // 0-0.4, chroma intensity for neutral colors
    };
    accessibilityThresholds: {
      AA: number; // WCAG AA contrast ratio (typically 4.5)
      AAA: number; // WCAG AAA contrast ratio (typically 7.0)
    };
    conversionOptions: {
    ...
    brandSpecific?: {
      [brandName: string]: {
        lightnessAdjustment?: number; // -20 to +20, lightness modifier for brand
        chromaMultiplier?: number; // 0.5 to 2.0, chroma intensity multiplier
        hueShift?: number; // 0-360, hue rotation degrees
      };
    };
    colorGeneration?: {
      rampSteps: number; // Number of steps in generated color ramps (5-11 typical)
      preserveKeyColors: boolean; // Whether to preserve manually defined key colors
      algorithmicGeneration: boolean; // Enable automated color ramp generation
    };
  };
}
 ...
 */
export const DEFAULT_COLOR_LIBRARY_CONFIG: ColorLibraryConfig = {
  colorLibrary: {
    colorSpace: 'oklch',
    lightnessRange: {
      min: 15,
