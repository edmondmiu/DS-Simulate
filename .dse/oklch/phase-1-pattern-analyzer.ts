...
interface ColorFamily {
  name: string;
  colors: { [key: string]: string }; // step -> hex
  oklchData: { [key: string]: OKLCHColor }; // step -> OKLCH
  lightnessProgression: number[]; // L values in order
  stepDifferences: number[]; // Difference between consecutive steps
  consistency: {
    averageStepSize: number;
    standardDeviation: number;
    smoothness: number; // Lower = more consistent
  };
}
...
export class Phase1PatternAnalyzer {
   ...
   */
  private static extractColorFamily(tokens: any, familyName: string): ColorFamily | null {
    ...
    const oklchData: { [key: string]: OKLCHColor } = {};
    ...
    Object.keys(family).forEach(colorKey => {
      ...
      if (colorData && colorData.$value) {
        const step = this.extractStepNumber(colorKey);
        if (step !== null) {
          colors[step.toString()] = colorData.$value;
          try {
            oklchData[step.toString()] = BaseColorAnalyzer.hexToOKLCH(colorData.$value);
          } catch (error) {
    ...
    const smoothness = standardDeviation / Math.abs(averageStepSize); // Lower = more consistent
   ...
   */
  private static extractStepNumber(colorKey: string): number | null {
   ...
   */
  public static async analyzePatterns(): Promise<PatternAnalysis> {
    ...
    const goodFamilies: ColorFamily[] = [];
    const problematicFamilies: ColorFamily[] = [];

    // Analyze good families
    console.log('📊 Analyzing "Good" Dark Mode Families:');
    for (const familyName of this.GOOD_FAMILIES) {
      const family = this.extractColorFamily(tokens, familyName);
      if (family) {
    ...
    for (const familyName of this.PROBLEMATIC_FAMILIES) {
      const family = this.extractColorFamily(tokens, familyName);
      if (family) {
   ...
   */
  public static generateHTMLVisualization(analysis: PatternAnalysis): string {
    const generateFamilyHTML = (family: ColorFamily, isGood: boolean) => {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  Phase1PatternAnalyzer.analyzePatterns().then(analysis => {
    // Generate HTML visualization
    const html = Phase1PatternAnalyzer.generateHTMLVisualization(analysis);
